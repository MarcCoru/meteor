...
def prepare_classification_model(nclasses, inplanes=15, norm="tbn", avg_pool=True):
    return ResNet(inplanes=inplanes, out_features=nclasses, normtype=norm, avg_pool=avg_pool)

def prepare_regular_model(name, n_classes, n_channels=13):
    if name == "resnet18":
        ...
        return model
    if name == "resnet50":
        model = models.resnet50(num_classes=n_classes)
        model.conv1 = nn.Conv2d(n_channels, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)
        return model
    if name == "resnet12":
        return ResNet(inplanes=n_channels, out_features=n_classes)
    else:
